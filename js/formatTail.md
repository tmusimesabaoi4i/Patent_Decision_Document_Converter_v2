...
- 改行コードは元テキストから推定（`\r\n` / `\r` / `\n`）し、結合に使う。
- ※ソース JSDoc の例では「番号だけ残して本文を削除」と読めるが、実コードは `m[5]`（ドット以降の本文）を保持し、ドット直後の空白 `m[4]` のみ落とす。
...
|---|---|---|---|
| 0 | 番号のパディング | 正規表現 `\s*(\d+)\s*(\/)\s*(\d+)(\s*-\s*)(\d+)\s*(\/)\s*(\d+)`（`n/m - x/y` 形式） | 1・2・4 個目の数字を幅 3、3 個目を幅 5 で右詰め（半角スペース埋め）。`/` はそのまま、`-` は前後空白を落とす |
...
| 8 | 調査した分野（IPC） | `/^・調査した分野[\s　]+IPC[\s　]+(.+)$/` | `・調査した分野　　ＩＰＣ　　` ＋末尾（ラベル間の空白を固定・`ＩＰＣ` 全角化） |
| 9 | 先行技術文献 | `/^・先行技術文献[\s　]+(.+)$/` | `・先行技術文献␣␣` ＋末尾（ラベルと本文の間を半角スペース 2 個に固定） |
| 10 | IPC コード行 | `/^([A-Za-z]\d{2}[A-Za-z].*)$/`（例 `H04B…` / `H04W…`） | 所定の全角スペースインデントを前置 |
...
| `convertFamilyInfoBodyLine` | 行頭の空白（半角/全角/タブ）をすべて除去 → `hwAlnum`（英数字のみ半角）→ 全角スペース **3 個** `　　　` を前置（※ソース JSDoc は「4 個」と書くが実コードは 3 個） |
